define(["backbone", "soundmanager2", "jquery", "json2", "log4javascript", "scripts/utils/number", "moment"],
	function(backbone, soundmanager2, jquery, json2, log4javascript, number_util, moment) {
		...
		LectureHallClient.ClockModel = Backbone.Model.extend({
			...
			syncToServer: function () {
				$.ajax({
					...
					error: function (jqXHR, textStatus, errorThrown) {
						logger.warn('Sync failed: '+ textStatus);
		     ...
		     */
		     setSimulatedTime: function (date) {
		     	var old = this.getTime();
		     	var offset = (date) ? parseInt(date.valueOf() - this.get('time')) : 0;
		     	this.set({simulatedOffset: offset});
		     	this.checkDateSwitch(old, this.getTime());
		     },

		     format: function (expression) {
		     	return moment(this.getTime()).format(expression);
		     }
		...
		LectureHallClient.EventModel = Backbone.Model.extend({
			...
			startDate: function () {
				return moment(this.get('startTime')).toDate();
			},
			 ...
			 */
			 endDate: function () {
			 	return moment(this.get('endTime')).toDate();
			 },
			 ...
			 */
			 isActive: function (timestamp) {
			 	...
			 	var endDate = this.endDateWithDiscussion().getTime();
		...
		LectureHallClient.EventModelList = Backbone.Collection.extend({
			...
			update: function () {
				...
				var next = this.find( function (model) {
			...
			setRoom: function (room) {
				...
				this.fetch({
					error: function () {
						logger.error("Unable to fetch contributions");
					},
					reset: true
				});
			},
			...
			url: function () {
				return LectureHallClient.Config.base_url + 'lhc/contributions?room=' + this.room + '&day=' + moment(this.clockModel.getTime()).format('YYYY-MM-DD');
			}
		});
		...
		LectureHallClient.EventTimelineView = Backbone.View.extend({
			...
			updateRender: function () {
				if (this.model.length > 0) {
					...
					var offset = firstModel.startDate().getTime() - this.options.clockModel.getTime();
				...
				if (this.model.getActive() === undefined) {
					...
					if (nextModel !== undefined) {
						...
						var timeToNext = moment.duration(nextModel.startDate().getTime() - this.options.clockModel.getTime());
						$('#next_event_info .time').text(timeToNext.hours().pad(1) + ":" + timeToNext.minutes().pad(2) + ":" + timeToNext.seconds().pad(2));
						$('#next_event_info .bar').css('width', LectureHallClient.Helper.millies2length(timeToNext.asMilliseconds()));
					}
					else {
				...
				$('#header #right #time').text(moment(this.options.clockModel.getTime()).format("ddd, HH:mm:ss", true));
			},
		...
		LectureHallClient.TalkView = Backbone.View.extend({
			...
			updateRender: function () {
				...
				if (inTalk) {
					// countdown in ms
					var countdown = moment.duration(this.options.eventModel.endDate().getTime() - this.options.clockModel.getTime());

					this.$('.countdown_time').toggleClass('discussion', false);
					this.$('.countdown_time').text(countdown.minutes().pad(2) + ":" + countdown.seconds().pad(2));

					if (countdown.asMilliseconds() >= 0) {
		    			// one minute
		    			if (countdown.minutes() == 1 && countdown.seconds() == 0) {
		    				LectureHallClient.Soundmanager.oneMinute.play() || logger.error("Could not play " + LectureHallClient.Soundmanager.oneMinute);
		    			}
		    			
		    			// discussion
		    			if (countdown.minutes() == 0 && countdown.seconds() == 0) {
		    				LectureHallClient.Soundmanager.discussion.play() || logger.error("Could not play " + LectureHallClient.Soundmanager.discussion);
		    			}
		    		}
		    	}
		    	else { /* inDiscussion */
		    		
		    		var countdown = moment.duration(this.options.eventModel.endDateWithDiscussion().getTime() - this.options.clockModel.getTime());
			...
			playRandomSong: function () {
				if (LectureHallClient.Soundmanager.music.length == 0) {
					logger.debug('No songs available');
				}
				...
				if (track !== undefined) {
					logger.debug('Starting track ' + track);
					track.play() || logger.error("Could not play " + track);
				}
				else {
					logger.warn('Breaksong at index ' + index + ' is undefined');
				}
			}
		});
		...
		LectureHallClient.BreakView = Backbone.View.extend({
			...
			initialize: function () {
				...
				if (this.model === undefined) {
					logger.error("View has no model");
				}
				else {
			...
			render: function () {
				if (this.model !== undefined) {
					...
					logger.warn('No eventModel defined');
				}
				return this;
			}
		});
		...
		LectureHallClient.BreakSlidesView = Backbone.View.extend({
			...
			initialize: function (options) {
				...
				if (this.model === undefined) {
					logger.error("BreakSlidesView initialized without a model");
				}
				else {
					this.model.bind('reset', this.render);
				}
			...
			updateRender: function (element) {		
				if (this.nextTalk !== undefined) {
					...
					var countdown = moment.duration(timeDiff);
					
					if (countdown.hours() == 0 && countdown.minutes() == 5 && countdown.seconds() == 0) {
						LectureHallClient.Soundmanager.gong && LectureHallClient.Soundmanager.gong.play() || logger.error("Could not play " + LectureHallClient.Soundmanager.gong);
					}
					
					if (countdown.hours() == 0 && countdown.minutes() == 3 && countdown.seconds() == 1) {
						if (LectureHallClient.Soundmanager.gong) {
							LectureHallClient.Soundmanager.gong.play() || logger.error("Could not play " + LectureHallClient.Soundmanager.gong);
							setTimeout(LectureHallClient.Soundmanager.gong.play, 5000);
						}
					}

					if (countdown.hours() == 0 && countdown.minutes() == 1 && countdown.seconds() == 20) {
						LectureHallClient.Soundmanager.oneMinute && LectureHallClient.Soundmanager.oneMinute.play() || logger.error("Could not play " + LectureHallClient.Soundmanager.oneMinute);
					}
				}
			},
		...
		LectureHallClient.AppView = Backbone.View.extend({
		    ...
		    simulateTime: function (mySQLTimedate) {
		    	this.options.clockModel.setSimulatedTime(moment(mySQLTimedate));
		    },
			...
			initialize: function (options) {
				...
				$.each(['gong', 'oneMinute', 'discussion'], function(index, value) {
					LectureHallClient.Soundmanager[value] = soundManager.createSound({
						...
						autoPlay: false,
						onload: function() {
							logger.debug('Loaded sound '+this.sID+'!');
						},
						volume: 70
					});
				});
